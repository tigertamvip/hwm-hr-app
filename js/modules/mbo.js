...
function renderWPTable(plan){
  ...
  if(plan.aiAnalysis && plan.aiAnalysis.trim()){
    html+='<div class="wp-feedback-textarea" id="aiAnalysisContent" style="background:#F4F0FF;border:1px solid #d4c8f0;font-size:13px;line-height:1.8;white-space:pre-wrap;transition:max-height 1s cubic-bezier(.25,.1,.25,1),opacity 1s cubic-bezier(.25,.1,.25,1),padding 1s cubic-bezier(.25,.1,.25,1);overflow:hidden;max-height:0;opacity:0;padding:0;margin-top:0">'+_h(plan.aiAnalysis)+'</div>';
  }else{
    html+='<div class="wp-feedback-empty" id="aiAnalysisContent" style="background:#F4F0FF;border:1px dashed #d4c8f0;transition:max-height 1s cubic-bezier(.25,.1,.25,1),opacity 1s cubic-bezier(.25,.1,.25,1),padding 1s cubic-bezier(.25,.1,.25,1);max-height:0;opacity:0;padding:0">点击上方「AI-分析建议」按钮，生成本周整体分析报告</div>';
  }
  html+='</div>';
