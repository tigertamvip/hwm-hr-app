-- 季度之星（V0.7.1fs 建表 + V0.7.1fu 点赞列 + V0.7.1fz 点赞人列）
-- 在 Supabase SQL Editor 执行一次即可（已建过表的只跑最后两行 ALTER 即可）
create table if not exists hwm_quarter_stars (
  ...
  liked_by jsonb default '[]'::jsonb, -- 点赞人 uid 数组（V0.7.1fz，每人一票）
...
alter table hwm_quarter_stars add column if not exists liked_by jsonb default '[]'::jsonb;
